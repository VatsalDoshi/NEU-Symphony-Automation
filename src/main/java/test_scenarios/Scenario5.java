...
public class Scenario5 {
	...
	public void runScenario(WebDriver driver, ExtentReports reports, ExtentTest test) throws Exception {
		...
		try {
			...
			js.executeScript("window.scrollBy(0, 2417)", "");
...
//					.until(ExpectedConditions.presenceOfElementLocated(By.id("\"[id^='ctl04_ctl'][id$='_buttonAtmc']\"")));
//					.until(ExpectedConditions.presenceOfElementLocated(By.id("\"[id^='ctl04_ctl'][id$='_buttonAtmc']\"")));
			js.executeScript("arguments[0].scrollIntoView(true);", addtoMyCalendarButton);
			Thread.sleep(2000);
//			wait.until(ExpectedConditions.visibilityOf(AddtoMyCalendarButton));
			js.executeScript("window.scrollBy(0, document.body.scrollHeight)");
			Thread.sleep(2000);
			...
			System.out.println("Successfully Completed TestScenario5");
		} catch (Exception e) {
